	...
	class ConsoleTester
    {
		static void Main(string[] args)
		{
			...
			for (int i = 0; i < 7; i++)
			{
				...
				fileP.InitialPartitionIndex = i;
				File file = SolitairePile.CreatePile<File>(fileP);
			...
			foreach (var suit in Suit.FULL_SUITS_LIST)
			{
				...
				Foundation found = SolitairePile.CreatePile<Foundation>(foundP);
				foundations.Add(found);
			}

			// Waste
			BuildStrategy wasteBS = new NoBuildStrategy();
			PileProperty wasteP = new PileProperty { InitialPartitionIndex = 3 };
			waste = SolitairePile.CreatePile<Waste>(wasteP);

			// Stock:
			BuildStrategy stockBS = new NoBuildStrategy();
			PileProperty stockP = new PileProperty { DealAmount = 3, RestockAllowance = Stock.INFINITE, AssociatedPiles = new List<SolitairePile> { waste } };
			stock = SolitairePile.CreatePile<Stock>(stockP);
